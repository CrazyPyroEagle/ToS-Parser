...
namespace ToSParser
{
    ...
    public static class ClientMessageParsers
    {
        ...
        public static void SendSystemMessage(this MessageParser parser, SystemCommand command, string args) => parser.Parse((byte)ClientMessageType.SEND_SYSTEM_MESSAGE, (buffer, index) => SEND_SYSTEM_MESSAGE.Build(buffer, index).Parse(command).Parse(args));
