...
namespace ToSParser
{
    ...
    public static class ServerMessageParsers
    {
        ...
        public static void PrivateMessage(this MessageParser parser, PrivateMessageType type, Player with, string message, Player? dst = null) => parser.Parse((byte)ServerMessageType.PRIVATE_MESSAGE, (buffer, index) => PRIVATE_MESSAGE.Build(buffer, index).Parse(type).Parse(with).Parse(dst == null, p => p.Parse(message), p => p.Parse(dst ?? default).Parse(message)));
        ...
        public static void CovenGotNecronomicon(this MessageParser parser, Player owner, Player? newOwner = null) => parser.Parse((byte)ServerMessageType.COVEN_GOT_NECRONOMICON, (buffer, index) => COVEN_GOT_NECRONOMICON.Build(buffer, index).Parse(newOwner != null).Parse(owner).Parse(newOwner == null, p => p, p => p.Parse(newOwner ?? default)));
        ...
        protected void Parse(byte[] buffer, int index, int length)
        {
            switch ((ServerMessageType)buffer[index++])
            {
                ...
                case ServerMessageType.ENABLE_SHOP_BUTTONS:
                    ...
                    EnableShopButtons();
                    break;
                case ServerMessageType.STEAM_POPUP:
                    if (SteamPopup == null) break;
                    ServerMessageParsers.STEAM_POPUP.Build(buffer, index, length).CheckPadding();
                    SteamPopup();
                    break;
